...
class  ModelShop{
    ...
    static public function priceLists(){
        $sap_database = $_ENV['DB_DATABASE_SAP'];
        $stmt = Connection::conectsap()->prepare("SELECT \"ListNum\" AS \"idsap\", \"ListName\" AS \"name\" FROM {$sap_database}.\"OPLN\" ORDER BY \"ListNum\" ");
    ...
    static public function linesForDocEntry($DocEntry){
        ...
        $stmt = Connection::conectsap()->prepare("SELECT 
                                                    ...
                                                    T2.\"ListNum\",
                                                    T1.\"TaxCode\"
                                                    FROM {$sap_database}.OPOR T0
                                                    INNER JOIN {$sap_database}.POR1 T1 ON T1.\"DocEntry\" = T0.\"DocEntry\"
    ...
    static public function productSAP($pricelist,$warehouse,$cardcode){
        ...
        $stmt = Connection::conectsap()->prepare("SELECT 
                                                    CASE 
                                                        WHEN T0.\"U_Medi\" = 'SI' THEN 'N'
                                                        WHEN T0.\"U_Medi\" = 'NO' THEN 'Y'
                                                    END AS \"VATLiable\",
                                                    T0.\"ItemCode\", 
                                                    T0.\"CardCode\", 
                                                    T0.\"CodeBars\", 
